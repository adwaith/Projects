...
void version_task ()
{
	...
	uint8_t buf_pos = 0; 
  if(log_g) printf("log:version_task PID=%d\n",nrk_get_pid());
  ...
  while(1) {
	// Wait for UART signal
		if(activate_uart_g == 1) {
			nrk_wait_until_next_period();
			continue;
		} else {
			vertsk_active_g = 1;
			if(log_g)printf("log:Version active\n");
			...
			while(nrk_uart_data_ready(NRK_DEFAULT_UART)!=0)
			{
				// Read Character
				c=getchar();
				version_buf[buf_pos++] = c; 	
				nrk_led_toggle(GREEN_LED);
				if(c =='$')
				{
					log_g = 1;
					version_buf[buf_pos++] = '\0';
					if(log_g) printf("log:Version: %s\n",version_buf);
					vertsk_active_g = 0;
				}
			}		
		}// else
...
void uart_task()
{
  ...
  while(1) {
	...
	if(activate_uart_g == 1 && other_active == 1 && vertsk_active_g == 0)
	{
	...
	else if(activate_uart_g == 0 || vertsk_active_g == 1)
	{
		nrk_wait_until_next_period();	
		continue;
	}
	...
	if(ack_received_g == 1)  
	{
		...
		while(nrk_uart_data_ready(NRK_DEFAULT_UART)!=0)
		{
			...
			if(c =='$')
			{
				...
				activate_uart_g = 0;
				other_active = 1;
				if(log_g) printf("log:Received Complete msg\n"); 
				// tx a packet				
...
void nrk_create_taskset()
{
  ...
  nrk_activate_task (&TaskFour);
	...
	TaskFive.task = version_task;
  nrk_task_set_stk( &TaskFive, Stack5,56);
  TaskFive.prio = 1;
  TaskFive.FirstActivation = TRUE;
  TaskFive.Type = BASIC_TASK;
  TaskFive.SchType = PREEMPTIVE;
  TaskFive.period.secs = 4;
  TaskFive.period.nano_secs = 0*NANOS_PER_MS;
