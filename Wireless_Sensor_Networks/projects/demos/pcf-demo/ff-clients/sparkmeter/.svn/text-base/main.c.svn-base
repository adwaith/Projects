...
true_power_thresh=((uint32_t)nrk_eeprom_read_byte(EEPROM_POWER_THRESH_ADDR_MSB))<<16 | 
		  ((uint32_t)nrk_eeprom_read_byte(EEPROM_POWER_THRESH_ADDR_MID))<<8 | 
		  (uint32_t)nrk_eeprom_read_byte(EEPROM_POWER_THRESH_ADDR_LSB);
...
// pack threshold into socket and v_p2p fields
    tx_pkt.payload[32] = true_power_thresh>>16;
    tx_pkt.payload[33] = true_power_thresh>>8;
    tx_pkt.payload[34] = true_power_thresh & 0xff;
