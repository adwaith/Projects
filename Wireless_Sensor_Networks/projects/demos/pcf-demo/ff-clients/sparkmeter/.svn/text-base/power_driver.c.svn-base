...
static int throttled=0, throttled_cnt=0;
...
static nrk_time_t throttle_timeout;

int8_t set_power_thresh(uint32_t power)
{
  true_power_thresh=power;
  nrk_eeprom_write_byte(EEPROM_POWER_THRESH_ADDR_MSB, (uint8_t)((uint32_t)true_power_thresh>>16)); 
  nrk_eeprom_write_byte(EEPROM_POWER_THRESH_ADDR_MID, (uint8_t)((uint32_t)true_power_thresh>>8));
  nrk_eeprom_write_byte(EEPROM_POWER_THRESH_ADDR_LSB, (uint8_t)((uint32_t)true_power_thresh&0xff));
throttled=0;
throttled_cnt=0;
...
{
  nrk_kprintf( PSTR( "Event Detector Task PID=" )); printf ("%u\r\n", nrk_get_pid ());
...
if(v==NRK_ERROR) nrk_kprintf( PSTR( "nrk_signal_register failed in event detector\r\n" ));
//ev_timeout.secs=10;
...
if(my_sigs!=0 && ticks_last>900)
{
  ...
  // If this is too slow, make a power of 2 instead...
...
if(true_power2>true_power_thresh)
	{
	throttled=1;
	throttled_cnt=0;
//	throttle_timeout.secs=30;
//	throttle_timeout.nano_secs=0;
	power_socket_throttle(0);
  	plug_led_green_clr();
	//nrk_led_clr(GREEN_LED);
//	nrk_wait(throttle_timeout);	
	// disable power
	// sleep 30 seconds
	// turn power back on
//	power_socket_enable(0);
...
if(throttled==1 && throttled_cnt<60) throttled_cnt++;
if(throttled_cnt>=60 )
	{
	power_socket_enable(0);
  	plug_led_green_set();
