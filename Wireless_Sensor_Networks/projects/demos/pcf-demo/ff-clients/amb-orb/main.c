...
#define SET_MAC  	0x0009
#define RADIO_CHAN	16
...
//  nrk_setup_uart (UART_BAUDRATE_115K2);
  nrk_setup_uart (UART_BAUDRATE_19K2);
//  setup_uart1(UART_BAUDRATE_19K2);

//  DDRE=0x2;
  //Default startup color = Red
  putc0('~'); putc0('G'); putc0('F'); 
  putc0('~'); putc0('R'); putc0('1'); 
...
void rx_task ()
{
  ...
  uint16_t cnt;
  int8_t v;
  uint8_t len, i,firstbyte,secondbyte;
uint8_t chan;
...
//  printf ("chan = %d\r\n", chan);
  ...
  while (1) {
    ...
    if (v == NRK_OK) {
      ...
      if(((rx_pkt.dst_mac&0xff) == (mac_address&0xff)) || rx_pkt.dst_mac==0xff) 
      {
      //printf ("len: %u\r\npayload: ", len);
      // for (i = 0; i < len; i++)
      //  printf ("%d ", rx_buf[i]);
	...
	//printf( "animation: %d\r\n", rx_pkt.payload[3] );
	putc0('~'); putc0('A'); 
	firstbyte= ( (rx_pkt.payload[2] + ( 37 * rx_pkt.payload[3])) / 94 ) + 32;
	secondbyte= ( (rx_pkt.payload[2] + ( 37 * rx_pkt.payload[3])) % 94 ) + 32;
	putc0(firstbyte);
	putc0(secondbyte);
	}
