...
nrk_task_type CNT_TASK;
NRK_STK cnt_task_stack[NRK_APP_STACKSIZE];
void cnt_task (void);
...
cpm=0;
cpm_temp=0;
 nrk_gpio_direction(NRK_UART1_RXD, NRK_PIN_INPUT);
 i=nrk_ext_int_configure( NRK_EXT_INT_2,NRK_RISING_EDGE, &radiation_int); 
...
void tx_task ()
{
  ...
  while (1) {
   	...
   	tx_pkt.payload[1] = 6;  // Key
	tx_pkt.payload[2]=cpm;
...
void nrk_create_taskset ()
{
  ...
  CNT_TASK.task = cnt_task;
  nrk_task_set_stk (&CNT_TASK, cnt_task_stack, NRK_APP_STACKSIZE);
  CNT_TASK.prio = 1;
  CNT_TASK.FirstActivation = TRUE;
  CNT_TASK.Type = BASIC_TASK;
  CNT_TASK.SchType = PREEMPTIVE;
  CNT_TASK.period.secs = 10;
  CNT_TASK.period.nano_secs = 10;
  CNT_TASK.cpu_reserve.secs = 20;
  CNT_TASK.cpu_reserve.nano_secs = 0;
  CNT_TASK.offset.secs = 0;
  CNT_TASK.offset.nano_secs = 0;
  nrk_activate_task (&CNT_TASK);
