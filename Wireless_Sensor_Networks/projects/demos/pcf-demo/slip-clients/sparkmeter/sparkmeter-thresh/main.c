...
int outlet_actuate (uint32_t mac, uint8_t socket, uint32_t state);
int send_ping ();
...
{
  char buffer[128];
  int status,v, cnt, i, size, socket;
  uint32_t  state;
  uint32_t target_mac;
  ...
  if (argc < 5) {
    printf ("Usage: server port mac-addr thresh [-v]\n");
    printf ("  mac-addr:  mac address in hex\n");
    printf ("  thresh:     1 for on, 0 for off\n");
    printf ("  Example:   ./plug-thresh localhost 5000 0x1 10000\n");
    exit (1);
  }
...
int outlet_actuate (uint32_t mac, uint8_t socket, uint32_t state)
{
  ...
  my_pkt.payload[1] = 3;        // KEY (3->power threshold) 
  my_pkt.payload[2] = socket;   // Outlet number 0/1
  my_pkt.payload[3] = state>>16;    // Outlet state 0/1
  my_pkt.payload[4] = state>>8;    // Outlet state 0/1
  my_pkt.payload[5] = state;    // Outlet state 0/1
  my_pkt.payload_len = 6;
