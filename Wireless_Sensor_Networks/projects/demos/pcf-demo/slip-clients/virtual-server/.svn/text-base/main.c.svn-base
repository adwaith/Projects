...
*/
  ...
  start_flag = 0;
printf( "Sending fake power packets...\n" );

while(1)
{
uint8_t checksum;

fake_buf[0]=2; // APP pkt
fake_buf[1]=15; // RSSI 
fake_buf[2]=4;   // SRC MAC
fake_buf[3]=0; 
fake_buf[4]=0; 
fake_buf[5]=0; 
fake_buf[6]=0;   // DST MAC
fake_buf[7]=0; 
fake_buf[8]=0; 
fake_buf[9]=0; 
fake_buf[10]=51;   // Payload Len
fake_buf[11]=1;  // ELEMENT
fake_buf[12]=1;  // KEY

total_secs++;
fake_buf[13] = (total_secs >> 24) & 0xff;
fake_buf[14] = (total_secs >> 16) & 0xff;
fake_buf[15] = (total_secs >> 8) & 0xff;
fake_buf[16] = (total_secs) & 0xff;
for(i=17; i<61; i++ ) fake_buf[i]=0;
fake_buf[17]=60;

server_non_blocking_rx (0);
checksum=0;
for(i=0; i<61; i++ ) checksum+=fake_buf[i];
fake_buf[62]=checksum;
server_tx (fake_buf, 63);
sleep(1);
}
