...
nrk_led_toggle(BLUE_LED);
		    ...
		    buf_to_pkt(slip_rx_buf,&rx_pkt);
		    if(rx_pkt.payload[1]==NW_CONFIG_SLEEP && rx_pkt.payload[2]==1 )
			{
			nrk_sw_wdt_stop(0);
			tdma_wakeup();
			nrk_sw_wdt_start(0);
...
tdma_set_slot_len_ms(20);
tdma_set_slots_per_cycle(64);
tdma_ttl_set(3);
...
nrk_led_set(GREEN_LED);
  while(1) {
	...
	if(v==NRK_OK)
	{
		// copy RSSI from tdma header into application packet
		buf_to_pkt(slip_tx_buf,&rx_pkt);
		rx_pkt.rssi=rx_tdma_fd.rssi;
                /*
                int i = 0;
                printf("\r\n");
                while (i < 37)
                ...
                printf("\r\n");
                */
		pkt_to_buf(&rx_pkt,&slip_tx_buf);
		if((rx_tdma_fd.seq_num!=last_seq_num) || (rx_tdma_fd.src!=last_mac) ) slip_tx ( slip_tx_buf, len );
		last_seq_num=rx_tdma_fd.seq_num;
		last_mac=rx_tdma_fd.src;
	}
	tdma_rx_pkt_release();
	nrk_led_clr(ORANGE_LED);
	nrk_sw_wdt_update(0); 
...
nrk_create_taskset()
{
  ...
  rx_task_info.period.nano_secs = 2*NANOS_PER_MS;
