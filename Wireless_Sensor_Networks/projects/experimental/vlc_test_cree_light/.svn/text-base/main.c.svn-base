...
*/
		...
		//Added to avoid any cahnge in delay after sync serial data is received
		//_nrk_os_timer_reset();
...
//				printf("\r\n%d",freq_f);
				...
				if(bcnt == 0)	//Let sync preamble length remain 1 symbol - so that preamble peak is clear
					...
					//nrk_wait_until_ticks(SYMBOL_LEN);
			}
		}
