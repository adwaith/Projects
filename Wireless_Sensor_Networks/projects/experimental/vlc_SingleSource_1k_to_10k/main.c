...
*******************************************************************************/
/*
vlc_SingleSource_1k_to_10k
# Sends out a single frequency for 1 minute. Power is matched by varying duty-cycle
#'a'-'j' for frequency 1k-10k
# Send z to terminate command. Send az for 1k, bz for 2k
# After one minute, it waits for serial input
*/
#include <nrk.h>
#include <include.h>
#include <ulib.h>
...
void Task2()
{
	...
	int8_t val;
	uint8_t tx_val;
	uint8_t Nbits = 10;//Two preamble bits and 8 data bits
	uint8_t index,ready;
...
*/
		...
		if(index==MSG_BUF_SIZE && c=='z' ) ready=1;
