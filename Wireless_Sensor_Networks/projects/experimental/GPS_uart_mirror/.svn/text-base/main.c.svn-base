...
// This program mirrors the RX and TX lines going to and from the GPS to the RX and 
// TX lines going from the atmega128rfa1 to the FTDI USB chip.
// RX1 is PD2  (connected to GPS TX)
// TX1 is PD3  (connected to GPS RX)
// RX0 is PE0  (connected to USB serial TX line)
// TX0 is PE1  (connected to USB serial RX line)


DDRE = 0x2;  // PE1 (TX0 output, RX0 as input)
DDRD = 0x8;  // PD3 (TX1 ouputi, RX1 as input)

// Disable the UART (this is enabled by the bootloader)
UCSR0B = 0;
UCSR1B = 0;

while(1)
{
  // Read RX1 and mirror on TX0
  if((PIND & 0x4)!=0 ) PORTE=0x2;
  else PORTE=0;

  // Read RX0 and mirror on TX1
  if((PINE & 0x1)!=0 ) PORTD=0x8;
   else PORTD=0x0;

}

// We never get here since we are doing this really simple mirroing thing...
