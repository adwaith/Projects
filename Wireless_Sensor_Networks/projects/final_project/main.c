...
int main ()
{
  ...
  {
	node[i].mac = 0;
	node[i].existing_version = 0;
	node[i].new_version = 0;
	node[i].flag = 0;
  }
  node[MAC_ADDR].mac = MAC_ADDR;
  node[MAC_ADDR].new_version = 1;
			...
			switch(msg_type)
			{
				case 1:
					if (!connection_flag)
					{
						...
						{
							printf("Sending connection request to node %d\r\n", sender);	
							request_flag = 1;
							sprintf(tx_buf, "2:%d:%d:0",MAC_ADDR, sender);
				...
				case 2:
					if (!connection_flag)
					{
						if (sender > MAC_ADDR)
						{
							printf("Sending connection approval to node %d\r\n", sender);
							connection_flag = 1;
							sprintf(tx_buf, "3:%d:%d:0:", MAC_ADDR, sender);
							bmac_tx_pkt(tx_buf, strlen(tx_buf));
							request_flag = 0;
						}
						if (sender < MAC_ADDR)
				...
				case 3:
					...
					{
						connection_flag = 1;
						sprintf(tx_buf, "4:%d:%d:0", MAC_ADDR, sender);
						bmac_tx_pkt(tx_buf, strlen(tx_buf));
					}
					break;
				case 4:
					sprintf(tx_buf, "5:%d:%d:abc-from-%d", MAC_ADDR, sender, MAC_ADDR);
					bmac_tx_pkt(tx_buf, strlen(tx_buf));
					connection_flag = 0;
					break;	
				case 5:
					node[sender].existing_version = node[sender].new_version;
					printf("Msg transfer from %d to %d done\r\n", sender, MAC_ADDR);
					connection_flag = 0;
					break;
				default:
//					printf("Invalid Packed %s\r\n", local_rx_buf);
...
void nrk_create_taskset ()
{
  ...
  DISCOVER_TASK.period.nano_secs = 200 * NANOS_PER_MS;
  DISCOVER_TASK.cpu_reserve.secs = 0;
  DISCOVER_TASK.cpu_reserve.nano_secs = 0 * NANOS_PER_MS;
