...
CFLAGS = -g -D NANORK -D NODE_ADDR=$(NODE_ADDR) -D PHOENIX -O$(OPT) \
...
#LDFLAGS += -Wl,--section-start=.bootloader_ws_flash_write_page=0x1F0C8

#LDFLAGS += -Wl,--section-start=.bootloader_ws_flash_read_page=0x1F000

#LDFLAGS += -Wl,--section-start=.bootloader_ws_flash_read_byte=0x1F086

#LDFLAGS += -Wl,--section-start=.bootloader_copy_section=0x1F1BA

#LDFLAGS += -Wl,--section-start=.bootloader_add_packet_to_page=0x1F2B8

#LDFLAGS += -Wl,--section-start=.bootloader_commit_page=0x1F2E2
...
# Default target.
all: begin gccversion sizebefore $(TARGET).elf $(TARGET).bin $(TARGET).hex $(TARGET).eep \
